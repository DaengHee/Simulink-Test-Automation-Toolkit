...
- At the start of each conversation in this repo, fetch the `upstream` remote (`Huelune/Simulink-Test-Automation-Toolkit`) and check whether `upstream/feat/harness-template-clone` has new commits beyond the local `feat/harness-template-clone` branch. If so, tell the user and offer to fast-forward the local branch (and `main`/`dev` if the user wants) to the new commit before continuing other work.
