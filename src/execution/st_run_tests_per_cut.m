...
reportZip = fullfile(evidenceDirectory, 'TestReport.zip');
coverageResultFile = fullfile(evidenceDirectory, 'CoverageResult.cvt');
evidencePath = fullfile(evidenceDirectory, 'evidence.json');
delete_if_present(reportZip);
delete_if_present(coverageResultFile);
...
try
    ...
    st_log(cfg, 'INFO', ...
        'Standalone package report capture start | CUT=%s | Model=%s', ...
        char(string(row.CUTName)), modelName);
    sltest.testmanager.report(resultObj, reportZip, ...
        'Title', [char(string(row.CUTName)) ' Test Report'], ...
        ...
        'LaunchReport', false);
    if ~isfile(reportZip)
        error('simtest:StandalonePackageEvidenceReportMissing', ...
            'Test Manager did not create the package report: %s', ...
            reportZip);
    end
    st_log(cfg, 'INFO', ...
        'Standalone package report capture complete | CUT=%s | File=%s', ...
        char(string(row.CUTName)), reportZip);
    ...
    % cvsave embeds the source model's info and fails with
    % cvi.ReportUtils.checkModelLoaded:ModelNotOpen once the model is
    % closed, so the .cvt must also be captured here while it is open --
    % PACKAGE runs well after every execution model has been closed.
    ...
    st_save_coverage_result(coverageResultFile, coverageObjects, cfg);
    if ~bdIsLoaded(modelName) || ...
            ~same_path(get_param(modelName, 'FileName'), modelFile)
        error('simtest:StandalonePackageEvidenceModelLost', ...
            ['Execution model changed or closed after coverage result ' ...
             'capture. Expected=%s'], modelFile);
    end
    ...
    evidence = struct( ...
        ...
        'ReportZip', reportZip, ...
        'ReportZipSHA256', st_file_signature(reportZip).SHA256, ...
        'CoverageResult', coverageResultFile, ...
        'CoverageResultSHA256', st_file_signature(coverageResultFile).SHA256, ...
